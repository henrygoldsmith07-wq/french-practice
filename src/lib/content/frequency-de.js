...
const band1 = b(1, `der / die / das	the
ein / eine	a / an
und	and
oder	or
aber	but
...
// ── Band 5 — home & food ─────────────────────────────────────────────────────
const band5 = b(5, `das Haus	house
...
const band6 = b(6, `der Körper	body
der Kopf	head
das Gesicht	face
das Auge	eye
das Ohr	ear
...
const band7 = b(7, `die Arbeit	work
der Job	job
...
const band8 = b(8, `die Reise	trip / journey
der Urlaub	holiday
der Zug	train
der Bahnhof	station
der Bus	bus
...
const band10 = b(10, `essen	to eat
trinken	to drink
schlafen	to sleep
aufstehen	to get up
arbeiten	to work
...
zweite	second
letzte	last`);
const colours = b(6, `die Farbe	colour
rot	red
blau	blue
grün	green
gelb	yellow
...
rosa	pink
lila	purple`);
const animals = b(8, `das Tier	animal
der Hund	dog
die Katze	cat
das Pferd	horse
die Kuh	cow
...
die Spinne	spider
die Biene	bee`);
const techMedia = b(8, `das Fernsehen	television
der Film	film
die Musik	music
das Lied	song
das Radio	radio
...
const feelings = b(7, `das Gefühl	feeling
die Liebe	love
die Angst	fear
die Freude	joy
die Wut	anger
...
const abstract = b(9, `die Wahrheit	truth
...
die Regel	rule
die Ordnung	order`);
const moreConnectors = b(4, `also	so / therefore
deshalb	that’s why
...
const verbs2 = b(10, `bekommen	to get / receive
bringen	to bring
setzen	to set / put
legen	to lay
stellen	to place
...
const professions = b(7, `der Arzt	doctor
...
const cityPlaces = b(8, `das Geschäft	shop
...
die Treppe	stairs`);
const sports = b(9, `der Sport	sport
der Fußball	football
das Spiel	match / game
die Mannschaft	team
der Ball	ball
das Tor	goal
schwimmen	to swim
...
der Sieg	victory`);
const houseTools = b(7, `das Werkzeug	tool
der Hammer	hammer
das Messer	knife
die Schere	scissors
die Uhr	watch / clock
...
nah	near
genau	exactly`);
const timeExtra = b(6, `die Sekunde	second
der Frühling	spring
der Sommer	summer
der Herbst	autumn
der Winter	winter
...
const bodyExtra = b(8, `die Schulter	shoulder
das Knie	knee
der Ellbogen	elbow
die Haut	skin
das Blut	blood
...
const verbs3 = b(4, `anfangen	to start
aufhören	to stop
...
const verbs4 = b(6, `verbessern	to improve
...
servieren	to serve
bestellen	to order`);
const verbs5 = b(7, `denken an	to think of
...
const foodEx = b(5, `die Banane	banana
die Orange	orange
die Zitrone	lemon
...
der Joghurt	yoghurt
die Mahlzeit	meal`);
const houseEx = b(6, `das Wohnzimmer	living room
...
const natureEx = b(7, `der Stein	stone
der Sand	sand
die Erde	earth / soil
das Gras	grass
das Blatt	leaf
...
const animalsEx = b(8, `das Kaninchen	rabbit
der Hase	hare
...
das Zebra	zebra
das Kamel	camel`);
const adjEx = b(6, `dick	thick / fat
dünn	thin
breit	wide
schmal	narrow
tief	deep
...
ekelhaft	disgusting
gesund	healthy`);
const adjEx2 = b(8, `freundlich	friendly
höflich	polite
unhöflich	rude
ehrlich	honest
nett	nice
...
const abstractEx = b(9, `die Zeit	time
der Raum	space
...
das Wachstum	growth`);
const emotionsEx = b(7, `die Stimmung	mood
die Laune	temper / mood
...
const businessMoney = b(7, `das Geschäft	business
der Handel	trade
der Markt	market
der Kunde	customer
der Verkauf	sale
...
const techScience = b(8, `die Wissenschaft	science
...
const artsMedia = b(8, `die Kunst	art
das Bild	picture
...
die Melodie	melody
der Rhythmus	rhythm`);
const education = b(7, `die Bildung	education
das Fach	subject
...
const societyLaw = b(9, `die Gesellschaft	society
...
const healthMed = b(7, `die Behandlung	treatment
...
taub	deaf
behindert	disabled`);
const transportEx = b(6, `die U-Bahn	underground / metro
...
const clothingEx = b(7, `die Jeans	jeans
das T-Shirt	T-shirt
die Bluse	blouse
der Anzug	suit
die Krawatte	tie
...
die Tasche	pocket
der Kragen	collar`);
const objectsEx = b(6, `das Ding	thing
...
der Schirm	umbrella`);
const communication = b(6, `das Gespräch	conversation
...
die Stille	silence
die Sprache	language`);
const familyRel = b(6, `die Eltern	parents
...
const cityBuild = b(7, `das Gebäude	building
der Turm	tower
das Schloss	castle
die Burg	fortress
die Fabrik	factory
...
die Gegend	area`);
const shoppingEx = b(6, `der Laden	shop
das Kaufhaus	department store
die Kasse	checkout
der Verkäufer	shop assistant
der Warenkorb	shopping basket
die Tüte	bag
...
const adverbs3 = b(5, `sicherlich	certainly
...
insgesamt	in total`);
const verbs6 = b(7, `gehören zu	to belong to
...
verbieten	to forbid
erlauben	to allow`);
const measures = b(5, `das Kilo	kilo
das Gramm	gram
das Pfund	pound
der Liter	litre
der Meter	metre
...
kein	none
genug	enough`);
const calendarEx = b(5, `der Januar	January
...
bald	soon
pünktlich	on time`);
const materials = b(8, `das Holz	wood
das Eisen	iron
der Stahl	steel
das Gold	gold
das Silber	silver
...
die Asche	ash
der Dampf	steam`);
const directions = b(4, `der Norden	north
der Süden	south
der Osten	east
der Westen	west
oben	top / above
...
die Nähe	proximity
die Ferne	distance`);
const miscNouns = b(9, `die Gelegenheit	opportunity
...
const verbs7 = b(8, `erreichen	to reach
verpassen	to miss
erlauben	to permit
...
const verbs8 = b(9, `bedeuten	to signify
...
verbinden	to link`);
const geography = b(7, `die Welt	world
...
die Kultur	culture
die Hauptstadt	capital`);
const plants = b(8, `die Pflanze	plant
der Busch	bush
die Hecke	hedge
die Palme	palm tree
die Eiche	oak
...
der Garten	garden
der Park	park`);
const kitchenEx = b(6, `das Geschirr	dishes
...
der Imbiss	snack`);
const hobbies = b(7, `das Hobby	hobby
...
das Reisen	travelling
das Spiel	game
die Karte	playing card
das Schach	chess
das Puzzle	puzzle
der Verein	club
das Mitglied	member
...
der Preis	prize
die Medaille	medal`);
const adjEx3 = b(8, `nützlich	useful
nutzlos	useless
praktisch	practical
bequem	comfortable
...
const weatherEx = b(6, `sonnig	sunny
wolkig	cloudy
regnerisch	rainy
windig	windy
neblig	foggy
...
const jobsEx = b(8, `der Bäcker	baker
der Metzger	butcher
...
const bodyEx2 = b(7, `das Gesicht	face
...
die Stimme	voice
der Puls	pulse`);
const mindWords = b(9, `der Geist	mind / spirit
die Seele	soul
der Verstand	reason / intellect
...
const moreNouns = b(9, `das System	system
...
der Wert	value
die Qualität	quality
die Eigenschaft	property / trait
das Merkmal	feature`);

const verbs9 = b(8, `ziehen	to pull
...
schütteln	to shake
umarmen	to hug`);
const officeEx = b(7, `die Akte	file
der Ordner	folder
das Formular	form
der Stempel	stamp
...
die Pause	break
die Schicht	shift
der Urlaub	leave / holiday`);
const routineEx = b(5, `aufwachen	to wake up
duschen	to shower
...
besetzt	occupied
verfügbar	available`);
const phrasesEx = b(3, `guten Morgen	good morning
guten Tag	good day
...
const moreNouns2 = b(9, `der Anlass	occasion
das Ereignis	event
