...
// ── Band 2 — top verbs ───────────────────────────────────────────────────────
const band2 = b(2, `ser	to be (permanent)
estar	to be (state)
haber	to have (aux.)
tener	to have
hacer	to do / make
...
// ── Band 5 — home & food ─────────────────────────────────────────────────────
const band5 = b(5, `la casa	house / home
...
const band6 = b(6, `el cuerpo	body
la cabeza	head
la cara	face
el ojo	eye
la oreja	ear
...
const band7 = b(7, `el trabajo	work
el empleo	job
...
const band10 = b(10, `comer	to eat
beber	to drink
dormir	to sleep
...
segundo	second
último	last`);
const colours = b(6, `el color	colour
rojo	red
azul	blue
verde	green
amarillo	yellow
...
rosa	pink
morado	purple`);
const animals = b(8, `el animal	animal
el perro	dog
el gato	cat
el caballo	horse
la vaca	cow
...
la araña	spider
la abeja	bee`);
const techMedia = b(8, `la televisión	television
la película	film
la música	music
la canción	song
la radio	radio
...
const feelings = b(7, `el sentimiento	feeling
el amor	love
el miedo	fear
la alegría	joy
la rabia	anger
...
const abstract = b(9, `la verdad	truth
la libertad	freedom
el poder	power
...
la regla	rule
el orden	order`);
const moreConnectors = b(4, `así que	so
por eso	that’s why
sin embargo	however
aunque	although
sino	but rather
...
const verbs2 = b(10, `recibir	to receive
traer	to bring
poner	to put
sacar	to take out
colocar	to place
...
crecer	to grow
nacer	to be born`);
const professions = b(7, `el médico	doctor
la enfermera	nurse
el profesor	teacher
...
const cityPlaces = b(8, `la tienda	shop
...
el ascensor	lift
la escalera	stairs`);
const sports = b(9, `el deporte	sport
el fútbol	football
el partido	match
el equipo	team
la pelota	ball
...
la victoria	victory`);
const houseTools = b(7, `la herramienta	tool
el martillo	hammer
...
la nevera	fridge
el horno	oven`);
const adverbs2 = b(5, `pronto	soon
de repente	suddenly
por fin	finally
...
despacio	slowly
deprisa	quickly`);
const timeExtra = b(6, `el segundo	second
la primavera	spring
el verano	summer
el otoño	autumn
el invierno	winter
...
const bodyExtra = b(8, `el hombro	shoulder
la rodilla	knee
el codo	elbow
la piel	skin
la sangre	blood
...
const verbs3 = b(4, `empezar	to start
parar	to stop
continuar	to continue
llegar	to arrive
salir	to depart
subir	to get on / go up
...
const verbs4 = b(6, `mejorar	to improve
comparar	to compare
elegir	to choose
...
servir	to serve
pedir	to order`);
const verbs5 = b(7, `pensar en	to think of
...
soñar	to dream
despertar	to wake`);
const foodEx = b(5, `el plátano	banana
la naranja	orange
el limón	lemon
la fresa	strawberry
la cereza	cherry
...
el yogur	yoghurt
la comida	meal`);
const houseEx = b(6, `el salón	living room
...
el vecino	neighbour
el casero	landlord`);
const natureEx = b(7, `la piedra	stone
la arena	sand
la tierra	soil
la hierba	grass
la hoja	leaf
...
const animalsEx = b(8, `el conejo	rabbit
la liebre	hare
la ardilla	squirrel
el zorro	fox
el lobo	wolf
...
distinto	different
único	unique`);
const abstractEx = b(9, `el espacio	space
la cantidad	quantity
el número	number
la parte	part
el conjunto	whole / set
la mitad	half
el final	end
...
const emotionsEx = b(7, `el humor	mood
...
const businessMoney = b(7, `el negocio	business
el comercio	trade
el cliente	customer
la venta	sale
la compra	purchase
...
la acción	share / stock
el gerente	manager
el empleado	employee
el departamento	department
la reunión	meeting
el pedido	order`);
const techScience = b(8, `la ciencia	science
...
const artsMedia = b(8, `el arte	art
el cuadro	painting
el dibujo	drawing
...
el ritmo	rhythm
el teatro	theatre`);
const education = b(7, `la educación	education
...
la biblioteca	library`);
const societyLaw = b(9, `la sociedad	society
el gobierno	government
el estado	state
el pueblo	people / nation
...
la fe	faith
la nación	nation`);
const healthMed = b(7, `el tratamiento	treatment
...
la sangre	blood`);
const transportEx = b(6, `el metro	underground
el tranvía	tram
el taxi	taxi
el camión	lorry
la moto	motorbike
...
la señal	sign
el carné de conducir	driving licence
la gasolina	petrol
...
la aduana	customs`);
const clothingEx = b(7, `los vaqueros	jeans
la camiseta	T-shirt
la blusa	blouse
el traje	suit
la corbata	tie
...
el cuello	collar
la manga	sleeve`);
const objectsEx = b(6, `la cosa	thing
el objeto	object
la caja	box
la bolsa	bag
el paquete	parcel
...
la balanza	scales
el imán	magnet`);
const communication = b(6, `la conversación	conversation
...
el idioma	language
la respuesta	reply`);
const familyRel = b(6, `los padres	parents
...
const cityBuild = b(7, `el edificio	building
la torre	tower
el castillo	castle
...
la capital	capital
la zona	zone`);
const shoppingEx = b(6, `la tienda	shop
...
abierto	open
gratis	free`);
const adverbs3 = b(5, `ciertamente	certainly
...
de repente	suddenly`);
const verbs6 = b(7, `pertenecer a	to belong to
...
aceptar	to accept`);
const measures = b(5, `el kilo	kilo
el gramo	gram
el litro	litre
el metro	metre
...
demasiado	too much
la media	average`);
const calendarEx = b(5, `enero	January
febrero	February
marzo	March
abril	April
mayo	May
...
puntual	on time
a tiempo	in time`);
const materials = b(8, `la madera	wood
el hierro	iron
el acero	steel
el oro	gold
la plata	silver
...
la ceniza	ash
el vapor	steam`);
const directions = b(4, `el norte	north
el sur	south
el este	east
el oeste	west
arriba	up / above
...
cerca	near
lejos	far`);
const miscNouns = b(9, `la oportunidad	opportunity
la casualidad	coincidence
la suerte	luck
la esperanza	hope
el sueño	dream
el deseo	wish
...
const verbs7 = b(8, `alcanzar	to reach
perder	to miss
permitir	to permit
impedir	to prevent
proteger	to protect
...
deber	to owe
invertir	to invest`);
const verbs8 = b(9, `significar	to mean
...
resolver	to solve
analizar	to analyse`);
const geography = b(7, `el mundo	world
...
la capital	capital
la bandera	flag`);
const plants = b(8, `la planta	plant
el arbusto	bush
el seto	hedge
...
el jardín	garden
el huerto	orchard`);
const kitchenEx = b(6, `la vajilla	crockery
...
la merienda	snack`);
const hobbies = b(7, `el pasatiempo	hobby
...
const weatherEx = b(6, `soleado	sunny
nublado	cloudy
lluvioso	rainy
ventoso	windy
brumoso	foggy
...
const jobsEx = b(8, `el panadero	baker
...
el asesor	consultant
el director	director`);
const bodyEx2 = b(7, `la ceja	eyebrow
la pestaña	eyelash
la lengua	tongue
la mandíbula	jaw
la garganta	throat
...
el pulso	pulse
la cintura	waist`);
const mindWords = b(9, `la mente	mind
el alma	soul
...
el pensamiento	thought`);
const moreNouns = b(9, `el sistema	system
...
const verbs9 = b(8, `tirar	to pull
empujar	to push
levantar	to lift
bajar	to lower
rodar	to roll
...
señalar	to point
esconder	to hide`);
const officeEx = b(7, `el archivo	file
la carpeta	folder
el formulario	form
el sello	stamp
la firma	signature
...
el jefe	boss
la oficina	office`);
const routineEx = b(5, `despertarse	to wake up
ducharse	to shower
vestirse	to dress
desayunar	to have breakfast
ir al trabajo	to go to work
almorzar	to have lunch
volver a casa	to come home
...
const moreAdj = b(6, `cansado	weary
despierto	awake
hambriento	hungry
lleno	full
sediento	thirsty
...
agotado	sold out`);
const phrasesEx = b(3, `buenos días	good morning
...
const moreNouns2 = b(9, `la ocasión	occasion
...
el peligro	danger
el riesgo	risk
la oportunidad	chance
la sugerencia	suggestion
la decisión	decision
la opinión	view
el punto de vista	standpoint
la intención	intent
la mejora	improvement
el problema	problem
la solución	solution`);
const finalBatch = [
  ...b(5, `la cucharada	spoonful
la pizca	pinch
...
el equilibrio	balance`),
  ...b(8, `la asignatura	school subject
la historia	history
la geografía	geography
el arte	art class
la educación física	PE
la música	music class
la informática	computer science
la economía	economics
la filosofía	philosophy
la psicología	psychology
la medicina	medicine (field)
el derecho	law (field)`),
];

const topUp = [
  ...b(7, `el vaso de plástico	plastic cup
la jarra	jug
el termo	thermos
el delantal	apron
el trapo	cloth
...
la lejía	bleach
el cubo de basura	bin
la fregona	mop
la escoba	broom
el recogedor	dustpan`),
  ...b(8, `el gobierno local	local government
el alcalde	mayor
...
la semejanza	similarity`),
  ...b(6, `el vecindario	neighbourhood
la acera	pavement
la carretera	road
