...
const b = (rank, text, ipa = {}) =>
  text.split('\n').map((block) => {
    const [fr, en] = block.split('\t');
    return ipa[fr] ? { fr, en, rank, ipa: ipa[fr] } : { fr, en, rank };
  });

// ── Band 1 — grammatical core: articles, pronouns, top prepositions ──────────
const band1 = b(1, `le / la	the
les	the (plural)
un / une	a / an
des	some
de	of / from
...
// ── Band 4 — food & drink ────────────────────────────────────────────────────
const band4b = b(4, `nourriture	food
repas	meal
...
const band5b = b(5, `vêtements	clothes
chemise	shirt
pantalon	trousers
robe	dress
jupe	skirt
...
const band5c = b(5, `temps	time / weather
heure	hour / time
minute	minute
seconde	second
jour	day
journée	day (duration)
semaine	week
mois	month
année	year
matin	morning
...
été	summer
automne	autumn
hiver	winter`, { 'temps': '/tɑ̃/', 'août': '/u(t)/' });

// ── Band 6 — city, places, transport ─────────────────────────────────────────
const band6 = b(6, `ville	city / town
rue	street
route	road
place	square / place
...
plage	beach
montagne	mountain
campagne	countryside`, { 'ville': '/vil/', 'voiture': '/vwatyʁ/', 'gare': '/ɡaʁ/' });

// ── Band 6 — nature & weather ────────────────────────────────────────────────
const band6b = b(6, `nature	nature
ciel	sky
soleil	sun
lune	moon
étoile	star
...
abeille	bee
papillon	butterfly
araignée	spider`, { 'soleil': '/sɔlɛj/', 'œil': '/œj/', 'chien': '/ʃjɛ̃/', 'oiseau': '/wazo/' });

// ── Band 7 — work, school, money, tech ───────────────────────────────────────
const band7 = b(7, `travail	work
métier	job / trade
patron	boss
employé	employee
entreprise	company
...
const band8 = b(8, `grand	big / tall
petit	small
bon	good
mauvais	bad
...
sûr	sure / safe
dangereux	dangerous
calme	calm`, { 'vieux': '/vjø/', 'chaud': '/ʃo/', 'heureux': '/øʁø/' });

// ── Band 9 — numbers, quantities, question words ─────────────────────────────
const band9 = b(9, `zéro	zero
un	one
deux	two
trois	three
quatre	four
...
personne	nobody
nombre	number
moitié	half`, { 'deux': '/dø/', 'six': '/sis/', 'neuf': '/nœf/', 'cinq': '/sɛ̃k/' });

// ── Band 10 — abstract nouns, feelings, useful extras ────────────────────────
const band10 = b(10, `idée	idea
problème	problem
solution	solution
raison	reason
exemple	example
...
bien sûr	of course`, { 'vie': '/vi/', 'pays': '/pei/', 'œil': '/œj/', 'oui': '/wi/' });

// ── Verbs, tier 2 — everyday actions ─────────────────────────────────────────
const verbs2 = b(4, `marcher	to walk
courir	to run
sauter	to jump
tomber	to fall
monter	to go up
...
const verbs3 = b(5, `expliquer	to explain
...
const verbs4 = b(6, `se réveiller	to wake up
se lever	to get up
...
const verbs5 = b(7, `diriger	to lead / manage
...
// ── Professions ──────────────────────────────────────────────────────────────
const jobs = b(6, `boulanger	baker
boucher	butcher
cuisinier	cook
chef	chef / boss
serveur	waiter
...
const household = b(6, `meuble	furniture
étagère	shelf
tiroir	drawer
placard	cupboard
évier	sink
...
const clothingx = b(6, `tenue	outfit
costume	suit
cravate	tie
nœud papillon	bow tie
jean	jeans
short	shorts
maillot	jersey / swimsuit
...
const colours = b(7, `clair	light (colour)
foncé	dark
vif	bright
pâle	pale
beige	beige
...
const tech = b(6, `technologie	technology
machine	machine
moteur	engine / motor
...
appareil	device
tablette	tablet
portable	laptop / mobile
smartphone	smartphone
caméra	camera
imprimante	printer
souris	mouse
casque	headphones / helmet
...
const business = b(7, `affaires	business
commerce	trade
économie	economy
marché	market
produit	product
...
const society = b(7, `société	society
...
const science = b(7, `science	science
recherche	research
...
const timex = b(6, `météo	weather forecast
...
// ── Travel & tourism ─────────────────────────────────────────────────────────
const travel = b(6, `voyage	journey
tourisme	tourism
touriste	tourist
vacances	holiday
séjour	stay
...
const emotions = b(7, `émotion	emotion
sentiment	feeling
humeur	mood
joie	joy
tristesse	sadness
...
const abstract = b(7, `vérité	truth
mensonge	lie
réalité	reality
rêve	dream
pensée	thought
...
const adverbs = b(5, `vraiment	really
presque	almost
...
const prepositions = b(5, `entre	between
parmi	among
pendant	during
depuis	since / for
jusqu’à	until
...
// ── Communication & media ────────────────────────────────────────────────────
const media = b(7, `communication	communication
...
// ── School & education ───────────────────────────────────────────────────────
const school = b(6, `école	school
collège	secondary school
lycée	high school
classe	class
élève	pupil
maître	teacher (primary)
...
const extras = b(6, `morceau	piece
partie	part
moitié	half
tiers	third
paire	pair
...
const verbs6 = b(7, `cuire	to cook (in oven)
bouillir	to boil
rôtir	to roast
griller	to grill
mélanger	to mix
...
const verbs7 = b(8, `saluer	to greet
...
const numbers2 = b(8, `soixante	sixty
...
dixième	tenth
douzaine	dozen
centaine	a hundred or so
millier	a thousand or so
quart	quarter
double	double
triple	triple
...
const places = b(7, `bâtiment	building
immeuble	building (block)
tour	tower
...
const transport = b(7, `camion	lorry / truck
camionnette	van
tramway	tram
scooter	scooter
...
const abstract2 = b(8, `honneur	honour
gloire	glory
respect	respect
dignité	dignity
...
const verbs8 = b(8, `exister	to exist
...
soutenir	to support
représenter	to stand for
concerner	to concern
impliquer	to imply
provoquer	to cause
entraîner	to lead to / train
