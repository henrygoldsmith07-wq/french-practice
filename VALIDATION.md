...
| **Listening / Reading Comprehension** | `listeningReadingValidation.js` stores app-vs-human pairs per track/text (`fp.comprehensionValidations.v1`): skill, item, the app's score, an independent teacher/exam mark, rater and source; a second different-rater mark enables double-marking (κ reuses the corpus implementation). Per-skill agreement: MAE, within-5/10, correlation with no-data/provisional/validated floors. Teacher entry lives in DevPanel → *Comprehension validation*; rows surface in Analytics → External validation. | **Not validated** — store starts empty; no human marks fabricated. |
...
comprehensionValidations: 0 → Not validated (listening + reading, per skill)
