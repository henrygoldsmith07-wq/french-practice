...
test('a vocabulary lapse enters the model; two clean recalls resolve it; one does not', async () => {
  ...
  let gap = storage.getLearnerErrors({ limit: 50 }).find((e) => e.id === 'vocabulary:livre');
  ...
  gap = storage.getLearnerErrors({ limit: 50 }).find((e) => e.id === 'vocabulary:livre');
  ...
  gap = model.entries.find((e) => e.id === 'vocabulary:livre');
  ...
  assert.equal(storage.getLearnerErrors({ limit: 50 }).some((e) => e.id === 'vocabulary:maison'), false);
});
...
test('a productive-mode lapse tracks its own gap key', async () => {
  ...
  assert.ok(storage.getLearnerErrors({ limit: 50 }).some((e) => e.id === 'vocabulary:livre::productive'));
});
